...
function hoverEffects() {
    ...
    //         $(this).css(
    //             "box-shadow",
    //             "2px 2px 22px -4px rgba(255,255,255,0.44)"
    //         )
...
function addWhiteNav() {
    ...
    $(".logo-img").attr(
        "src",
        "https://brendananning.wedding/files/img/logo.png"
    )
...
function addWhiteNavDesktop() {
    ...
    $(".navbar-right").css("top", "10px")
    $(".contact-icon.phone").attr(
        "src",
        "https://brendananning.wedding/files/img/icons/phone.png"
    )
    $(".contact-icon.email").attr(
        "src",
        "https://brendananning.wedding/files/img/icons/email.png"
    )
    $(".contact-icon.insta").attr(
        "src",
        "https://brendananning.wedding/files/img/icons/instagram.png"
    )
    $(".contact-icon.facebook").attr(
        "src",
        "https://brendananning.wedding/files/img/icons/fb.png"
    )
}
function addTransparentNav() {
    ...
    $(".logo-img").attr(
        "src",
        "https://brendananning.wedding/files/img/logo_white.png"
    )
    $(".navbar-right").css("display", "flex")
    $(".navbar-right").css("top", "40px")
    $(".contact-icon.phone").attr(
        "src",
        "https://brendananning.wedding/files/img/icons/phone_white.png"
    )
    $(".contact-icon.email").attr(
        "src",
        "https://brendananning.wedding/files/img/icons/email_white.png"
    )
    $(".contact-icon.insta").attr(
        "src",
        "https://brendananning.wedding/files/img/icons/instagram_white.png"
    )
    $(".contact-icon.facebook").attr(
        "src",
        "https://brendananning.wedding/files/img/icons/fb_white.png"
    )
}
...
function setMainElements() {
    ...
    if (
        $(window).width() < 991 &&
        document.title == "Brisbane Marriage Celebrant | Brendan Anning"
    ) {
        setTimeout(function () {
            $("#wedding-datepicker input").css(
                "width",
                $(".time-label").outerWidth() + "px"
            )
        }, 600)
    } else {
        $("#wedding-datepicker input").removeAttr("style")
...
function bindVelocity() {
    ...
    $('a[href*="#"]').on("click", function (e) {
        ...
        if (target == "#package-one-contact") {
            $("#message").val(
                "Hello, I'd like to book the South East Queensland package. "
            )
            target = "#contact"
            setTimeout(function () {
                $("#message").focus()
        ...
        if (target == "#package-two-contact") {
            $("#message").val(
                "Hello, I'd like to book the Destination package. "
            )
            target = "#contact"
            setTimeout(function () {
                $("#message").focus()
        ...
        if (target == "#package-three-contact") {
            $("#message").val(
                "Hello, I'd like to book the Sydney/Melbourne package. "
            )
            target = "#contact"
            setTimeout(function () {
                $("#message").focus()
...
Pace.on("done", function () {
    if ($(".pace-progress").attr("data-progress-text") == "100%") {
        ...
        setTimeout(function () {
            ...
            displayTl
                ...
                .staggerTo($(".title-text").children(), 1, { opacity: 1 }, 0.8)
            //build a scene
    ...
    $(window).scroll(function () {
        ...
        $(".parallax.contact h1").css(
            "opacity",
            1 - $(window).scrollTop() / 300
        )
    })
})
...
function initialiseScrollReveal() {
    ScrollReveal().reveal(
        ".testimonials .first",
        checkViewportTestimonialsHeaderFirst
    )
    ScrollReveal().reveal(
        ".testimonials .second",
        checkViewportTestimonialsHeaderSecond
    )
    ScrollReveal().reveal(".packages .first", checkViewportPackagesHeaderFirst)
    if ($(window).width() < 991) {
        ScrollReveal().reveal(
            ".packages .contract-text",
            checkViewportPackagesHeaderSecond
        )
    }
